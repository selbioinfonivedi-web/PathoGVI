...
Deeper reference material moved out of the main [README](../README.md) to keep that
short. Nothing here changes what the tool does — this is architecture, testing
philosophy, estimator internals, and known scientific caveats for anyone building on
or reviewing the tool, not needed just to run it.

**Contents:** [What input unlocks which index](#what-input-unlocks-which-index) ·
[Estimator options](#estimator-options) · [Packaging a release](#packaging-a-release) ·
...
so the composite actually sums nine terms:

```
GVI(t) = w₁·μ + w₂·Re + w₃·π + w₄·MB + w₅·dN/dS + w₆·GD + w₇·CAI + w₈·GC + w₉·RI     (Σw = 1)
```

That's the number the web UI's index picker and every JSON/CSV report use.
`IndexKey`'s javadoc has the full reasoning.
...
`dummy_data/` in the repo has a small example of all four optional inputs together,
...
| `dummy_data/` | A small synthetic dataset (15 taxa, a built-in molecular clock) exercising all four optional inputs |
